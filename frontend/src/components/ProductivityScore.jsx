export default function ProductivityScore({ report }) {
    ...
    const gradeColor = score >= 70 ? "#e2ff00" : score >= 50 ? "#ffd700" : "#ff6b6b"
        ...
        <div style={s.factors}>
          {factors.map(f => (
            <div key={f.label} style={s.factor}>
              <div style={{ ...s.factorDot, background: f.positive ? "#e2ff00" : "#ff6b6b" }} />
  ...
  const s = {
    ...
    scoreBar: { height: "3px", background: "rgba(255,255,255,0.08)", borderRadius: "2px", overflow: "hidden", marginBottom: "24px" },
